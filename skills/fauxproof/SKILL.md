...
1. When the user asks to save a photo, screenshot, graph, or image attached in Claude, call `open_picture_upload` immediately. Do not stop at or lead with an explanation that Claude cannot forward the attachment. The tool returns a clickable **Add a picture to FauxProof** action; tell the user to click it and select that same picture with **Add picture from device**. If `open_picture_upload` is unavailable, call `render_lockbox` and give the same instruction. Never ask for a public URL and never replace the picture with a text description.
